...
{
    // For each <action, callback> in real time events pool,
    // execute callback if something specific happens
    // (as described in Action's test method)
...
{
    // == Operator overloaded in Action
    // This lambda function is used to easily remove
    // a certain action from event lists.
